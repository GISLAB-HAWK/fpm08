...
In dieser Übung wird wird vorgestellt wir ansprechendes Kartenlayouts erstellen und als Graphik
exportiert werden können. Das Kartenlayout soll ein Kartenfeld, einen Titel, eine Legende, einen
Nordpfeil, Gitterkoordinaten und Metadaten enthalten. QGIS bietet hierzu sehr viele Optionen, die
wir im Rahmen des Kurses nur kurz anschneiden können.
...
1.  Erstellen eine Luftbildkarte

2.  Erstellen einer Bestandesshöhenkarte

3.  Erstellen einer Geländehöhenkarte
...
Geoinformation (TLBG) im FoA Heilligenstadt. Kopieren sie sich die Luftbilder aus der LE05. Der Teil
des FoA, den wir bearbeiten möchten, wird durch folgende Kacheln abgedeckt: - 32580_5691 -
32580_5692 - 32581_5691 - 32581_5692

Sollten sie die Dateien nicht mehr zur Verfügung haben, können sie den Dowload auch wie in in
@sec-le05_download beschrieben wiederholen.
...
Abt. 24, 25, 26, 27, 29, und 30.

Filtern sie dazu den Datensatz wie in Übung @sec-le05_filter vorgestellt.

Anschließen wählen sie eine geeingete Symbolisierung für die Außengrenzen der Bestände. Außerdem
sollen die Bestände nach dem Schema AbtUabtTF (z.B. 30a1) beschriftet werden. Dazu können sie unter
Eigenschaften-\>Symbolisierung-\>Beschriftung-\>Einzelne Beschriftungen wählen. Da es bisher kein
Feld in der Attributabelle des Datensatzes gibt das die Beschriftung enthält müssen wir einen
...
Für die Erstellung eines Kartenlayouts müssen zunächst die Rasterdaten entsprechend vorbereitet
werden. Führen sie dazu folgende Schritte durch:

1.) Erstellen sie jeweils für das DGM, das DOM und das DOP ein virtuelles Rastermosaik mit den vier
Kacheln. Schauen sie dazu auch unter @sec-mosaikieren.

2.) Berechnen sie ein normalisiertes Oberflächenmodell und speichern sie dieses und dem Namen
*ndom.tif* ab. Schauen sie dazu auch unter @sec-LE12_ndom.

Passen sie nun die Darstellung der drei Rastermosaike wie folgt an:

1.) Das DGM soll als Schummerungsbild dargestellt werden

2.) Dasn nDOM soll als Einkanalpseudofarbe mit der Farbpalette "viridis" dargestellt werden.
...
verschiedenen Karten wird der Kartenlayout-Manager verwendet. Sie finden diesen im Menü unter Datei.
Hier können sie ein neues Kartenlayout „Luftbild“ anlegen.
...
![Luftbildkarte](images/LE14_Luftbildkarte.png){alt="Luftbildkarte"}
